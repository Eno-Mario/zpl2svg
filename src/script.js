...
^FX QR Code Example
^FO20,850^BQN,2,8,L,7^FD123456789^FS
^FO200,850^BQN,2,5,M,0^FDhttps://example.com^FS
...
const update_mouse_pos = (e) => {
  ...
  const x = map(
    ...
    mouse_pos.width,
    state.viewBox.x,
    state.viewBox.x + state.viewBox.width
  );
  const y = map(
    mouse_pos.y,
    0,
    mouse_pos.height,
    state.viewBox.y,
    state.viewBox.y + state.viewBox.height
  );
...
render_element.addEventListener("touchmove", (e) => {
  ...
  const touch = JSON.stringify(
    [...e.touches].map((t) => ({
      x: t.clientX.toFixed(1),
      y: t.clientY.toFixed(1),
    }))
  );
...
const map = (value, in_min, in_max, out_min, out_max) =>
  ((value - in_min) * (out_max - out_min)) / (in_max - in_min || 1) + out_min;
...
render_element.addEventListener("mouseup", (e) => {
  if (isDragging) e.preventDefault();
  isDragging = false;
});
render_element.addEventListener("mouseleave", (e) => {
  isDragging = false;
});
render_element.addEventListener("touchend", (e) => {
  if (isDragging) e.preventDefault();
  isDragging = false;
});
...
const update_sizes = () => {
  ...
  if (state.element) {
    ...
    const new_width =
      innerWidth >= 1600 ? body_width * (2 / 3) - 40 : body_width - 50;
...
const update_svg = () => {
  ...
  timeout = setTimeout(
    () => {
      ...
      // @ts-ignore
      state.svg_content = zpl2svg(zpl, {
        width,
        height,
        custom_class: "custom-svg-window",
      });
      ...
      if (!exists && state.element && !state.viewBoxBase) {
        const viewBox =
          state.element.getAttribute("viewBox") || "0 0 1000 1000";
...
download_png.addEventListener("click", () =>
  export_png("label.png", state.svg_content)
);
...
const getLastCommits = async () => {
  ...
  const getCommits = async (username, repo, branch, limit = 5) => {
    const res = await fetch(
      `https://api.github.com/repos/${username}/${repo}/commits?sha=${branch}&per_page=${limit}`
    );
  ...
  try {
    ...
    try {
      ...
      if (load_new_commits)
        localStorage.setItem("commit_cache", JSON.stringify(stored_commits)); // Store commits in cache
      ...
      const data = commits
          ...
          (
            {
              ...
              author: { login, avatar_url, html_url: url_author },
              sha,
              html_url: url_commit,
            },
            i
        ...
        .map(
          ({
            ...
            `<a href="${url_commit}" target="_blank">${sha.substring(
              0,
              7
            )}...</a>`,
      ...
      data.forEach((commit) => {
        ...
        if (index_of_last_semicolon != -1)
          commit[1] = date.substring(0, index_of_last_semicolon);
      });
      table_element.innerHTML = `
                <table id="commits-table">
                    ...
                    ${data
                      .map(
                        ([index, date, commit, user, message]) =>
                          `<tr><td>${index}</td><td>${date}</td><td>${commit}</td><td><span>${user}</span></td><td>${message}</td></tr>`
                      )
                      .join("")}
                </table>`;
    } catch (error) {
      if (commits && commits.message) {
        ...
        table_element.innerHTML = `<p style="color: red;">Failed to load commits</p><p>${
